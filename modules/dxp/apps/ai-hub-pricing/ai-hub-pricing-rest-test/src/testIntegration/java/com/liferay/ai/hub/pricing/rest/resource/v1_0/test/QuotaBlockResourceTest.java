...
public class QuotaBlockResourceTest extends BaseQuotaBlockResourceTestCase {
	...
	public void testPostAccountQuotaBlockPurchase() throws Exception {
		...
		BigDecimal finalSize = BigDecimal.valueOf(RandomTestUtil.randomInt());
		String finalTransactionId = RandomTestUtil.randomString();
		...
		Assert.assertEquals(
			finalTransactionId, postQuotaBlock.getExternalReferenceCode());
		...
		Assert.assertTrue(
			BigDecimalUtil.eq(finalSize, postQuotaBlock.getRemainingBalance()));
		Assert.assertTrue(
			BigDecimalUtil.eq(finalSize, postQuotaBlock.getSize()));
		Assert.assertEquals(
			finalTransactionId, postQuotaBlock.getTransactionId());
		Assert.assertEquals(0.6, postQuotaBlock.getVersion(), 0.0001);
	}
	...
	protected QuotaBlock randomQuotaBlock() throws Exception {
		return new QuotaBlock() {
			{
				setSize(() -> BigDecimal.valueOf(RandomTestUtil.randomInt()));
