...
public class CrawlerRestController extends BaseRestController {
	...
	private List<JSONObject> _getInsightJSONObjects(
		String category,
		UnsafeSupplier<List<JSONObject>, Exception> unsafeSupplier) {
		...
		try {
			...
			_log.error(
				"Unable to get insights for category " + category, exception);
	...
	private void _patchSEOStudioScan(JSONObject seoStudioScanJSONObject)
		...
		if (state.equals(SEOStudioScanConstants.STATE_COMPLETED)) {
			...
			List<JSONObject> linksAndURLsInsightJSONObjects =
				_getInsightJSONObjects(
					"linksAndURLs",
					() ->
						_seoStudioLinksAndURLsInsightService.
							getInsightJSONObjects(crawlHits, domainURL));
			...
			for (JSONObject insightJSONObject :
					ListUtil.concat(
						linksAndURLsInsightJSONObjects,
						metadataInsightJSONObjects)) {

				_seoStudioService.postSEOStudioScanInsightsBatch(
					accountEntryId, insightJSONObject, seoStudioScanId);
			}

			if (ListUtil.isEmpty(linksAndURLsInsightJSONObjects) ||
				ListUtil.isEmpty(metadataInsightJSONObjects)) {

				_seoStudioService.patchSEOStudioScan(
					"Unable to get insights for one or more categories",
					seoStudioScanId, SEOStudioScanConstants.STATE_FAILED);
